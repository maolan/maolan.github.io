+++
title = "OSC"
description = "Reference for enabling Maolan OSC support, the full command and query address space, listener settings, and the maolan-osc helper binary."
layout = "osc"
+++

# Maolan OSC

An Open Sound Control surface covering transport, track
management, routing, plugins, automation, clip editing, MIDI
editing, MIDI learn, devices, offline bounce, and state queries.

Default listener: `0.0.0.0:9000`  
Command groups: `13`  
Query addresses: `15`

[Enabling](#enabling) [Listener](#listener) [Commands](#commands) [Queries](#queries) [Helper](#helper) [Notes](#notes)
...
There is **no `/maolan` prefix** on any OSC address. Arguments
use OSC type tags:
...
Complex structured data (clip data, MIDI notes, controllers,
tempo maps, modulators, audio-device settings, automation lanes)
is sent as JSON strings because the OSC implementation uses flat
typed arguments rather than bundles.
...
- `/clip/add s s i i i i i i i i s [s] [s]` — track, name, start, length, offset, input_channel, muted, fade_enabled, fade_in, fade_out, kind, optional source_name, optional preview_name
- `/clip/remove s s s` — track, kind, comma-separated indices
- `/clip/move s s i s i i i` — kind, from_track, from_index, to_track, to_offset, to_channel, copy
- `/clip/fade s i s i i i` — track, clip_index, kind, fade_enabled, fade_in_samples, fade_out_samples
...
Controller JSON objects use `sample`, `controller`, `value`, and
`channel`. SysEx JSON format:
...
- `/plugin/restore_state s s i s` — JSON state (`{"bytes":[...]}` for CLAP, `{"plugin_id":"...","component_state":[...],"controller_state":[...]}` for VST3)
- `/plugin/set_resource_dir s s i s`
- `/plugin/update_file_reference s s i i s` — track_name, format, instance_id, file_index, path
...
Track targets: `volume`, `balance`, `mute`, `solo`, `arm`,
`input_monitor`, `disk_monitor`.
...
- `/bounce/start s s i i s i` — track, output_path, start_sample, length_samples, automation_lanes JSON, apply_fader
- `/bounce/cancel s`
- `/bounce/cancel_all`
...
- `/query/transport` → `/response/transport i i f i i` (sample, playing, tempo, time_sig_num, time_sig_denom)
- `/query/meters` → `/response/meters ...`
- `/query/plugins s` → `/response/plugins s i (i s s s i)*` (track_name, plugin_count, instance_id, format, uri, name, bypassed)
- `/query/plugin_parameters s s i i` → `/response/plugin_parameters s i s s` (track_name, instance_id, format, parameters_json)
...
Plugin-list replies contain strings in the form `id|name` for
CLAP and VST3, `uri|name` for LV2.
